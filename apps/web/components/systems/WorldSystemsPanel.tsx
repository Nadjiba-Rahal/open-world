    ...
    <aside className={`systems ${open ? "is-open" : ""}`}>
      ...
      {open && <div className="systems-body">
        <div className="systems-tabs">
          {(["pack", "craft", "quests", "home", "journal", "social"] as SystemsTab[]).map((item) => <button type="button" className={tab === item ? "active" : ""} key={item} onClick={() => setTab(item)}>{item}</button>)}
        </div>
        ...
        {tab === "social" && <section className="systems-section">
          ...
          <div className="emote-grid">{EMOTE_CATALOG.map((emote) => <button type="button" key={emote.id} onClick={() => onEmote(emote.id)}><b>{emote.symbol}</b><span>{emote.name}</span></button>)}</div>
